...
MongoDOWN.prototype._open = function (options, callback) {
  ...
  var connect = function () {
    self._db = mongojs(self.location, [collection]);
    callback(null, self);
  };
...
MongoDOWN.prototype._get = function (key, options, callback) {
  this._db[collection].findOne({ _id: key }, function (err, doc) {
    if (err) return callback(err);
...
MongoDOWN.prototype._put = function (key, value, options, callback) {
  this._db[collection].update({ _id: key }, { _id: key, value: value }, { upsert: true }, callback);
};

MongoDOWN.prototype._del = function (key, options, callback) {
  this._db[collection].remove({ _id: key }, callback);
};
    ...
    switch (batch[0].type) {
      case 'put':
        ...
        for (var n = 0, l = batch.length; n < l; n++)
          self._db[collection].save({ _id: batch[n].key, value: batch[n].value }, next());
        break;
      case 'del':
        var keys = batch.map(function (e) { return e.key; });
        self._db[collection].remove({ _id: { $in: keys } }, commit);
        break;
...
MongoDOWN.prototype._approximateSize = function (start, end, callback) {
  this._db[collection].count({ _id: { $gte: start, $lte: end } }, callback);
};
...
var MongoIterator = function (db, options) {
  ...
  this._cursor = db._db[collection].find(query).sort({ _id: options.reverse ? -1 : 1 });
